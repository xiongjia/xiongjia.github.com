...
2. **用 Rustlings 练习** — 比被动看书效率高很多
3. **写代码 > 看书** — 每天写至少 50 行 Rust
4. **读懂编译器错误** — Rust 的错误信息是最好的老师
5. **从 CLI 工具入手** — 比 Web 项目简单，更容易上手
6. **不要过早接触 unsafe** — 99% 的代码不需要它
7. **用 clippy** — `cargo clippy` 教你的不仅是 lint，还有惯用法
