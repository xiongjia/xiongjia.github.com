...
### 阶段 2: 理解认证流程
3. `deps.py` - 依赖注入模式
4. `security.py` - JWT、密码哈希、TOTP、OIDC

### 阶段 3: 理解数据层
5. `models/models.py` - ORM 模型定义 (Trip, Place, User 关系)
6. `db/core.py` - 数据库初始化与迁移

### 阶段 4: 理解业务逻辑
7. `routers/trips.py` - 行程核心逻辑 (最复杂)
8. `routers/places.py` - 地点 CRUD
9. `routers/auth.py` - 认证流程

### 阶段 5: 理解前端
10. Angular services (`src/src/app/services/`) - API 调用
11. Angular pages (`src/src/app/pages/`) - 页面组件

### 阶段 6: 理解地图
12. `shared/map.ts` - Leaflet 地图初始化、标记、GPX 绘制
13. `utils/providers/base.py` - Provider 抽象接口
14. `utils/providers/osm.py` - OSM 搜索与路由
15. `routers/providers.py` - 地图 API 端点
