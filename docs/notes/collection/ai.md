---
icon: material/robot
hide:
  - tags
---

# :material-robot: AI

## Local AI

- [LocalAI](https://localai.io/) - 本地部署 AI 推理

## AI Code Tools

- [Claude Run](https://github.com/kamranahmedse/claude-run) - 将 Claude Code 作为 CLI 运行
- [how-pi-agent-works](https://github.com/cellinlab/how-pi-agent-works) - Pi Agent 原理分析
- [hello-agents](https://github.com/datawhalechina/hello-agents) - AI Agent 入门
- [pi-docs-playbook](https://github.com/enderzcx/pi-docs-playbook) - Pi Agent 文档
- [Claude Command Suite](https://github.com/qdhenry/Claude-Command-Suite) - Claude 命令集成套件
- [Claude-BugHunter](https://github.com/elementalsouls/Claude-BugHunter) - Claude Code bug hunting skill 包，82 个 skill、15 个 slash command、覆盖 24 类漏洞的 681 条披露报告模式

## AI Skills & MCP

- [claude-scientific-skills](https://github.com/K-Dense-AI/claude-scientific-skills) - 科研 skill
- [AI Skills for Everyone](https://github.com/datawhalechina/ai-skills-for-everyone) - AI Skill 合集
- [best-skills](https://github.com/xstongxue/best-skills) - Skill 排行榜
- [OpenCodeCN Skills](https://www.opencodecn.com/docs/skills/top-6-skill-collections) - Skill 集合
- [excalidraw-mcp](https://github.com/excalidraw/excalidraw-mcp) - Excalidraw MCP 服务

## AI CLI

- [bub](https://github.com/bubbuild/bub) - AI CLI 工具
- [pinchtab](https://github.com/pinchtab/pinchtab) - AI Golang 工具
- [RTK](https://github.com/rtk-ai/rtk) / [rtk-ai.app](https://www.rtk-ai.app/) - Rust Token Killer，将常见开发命令的 LLM token 消耗降低 60-90%，单 Rust 二进制、零依赖

## AI Audio

- [whisper](https://github.com/openai/whisper) - 语音转文字
- [GPT-SoVITS](https://github.com/RVC-Boss/GPT-SoVITS) - 文字转语音

## Neovim AI

- [llm.nvim](https://github.com/Kurama622/llm.nvim)
- [ChatGPT.nvim](https://github.com/jackMort/ChatGPT.nvim)
- [codecompanion.nvim](https://github.com/olimorris/codecompanion.nvim)
- [vim-ai](https://github.com/madox2/vim-ai)

## AI Learning

- [Agentic Design Patterns](https://github.com/xindoo/agentic-design-patterns) - Agent 设计模式
- [How AI Impacts Skill Formation](https://arxiv.org/abs/2601.20245) - AI 对技能形成的影响

## LLM Observability

- [langfuse](https://github.com/langfuse/langfuse) - 开源 LLM 可观测性平台（evals、指标监控、prompt 管理、playground）

## Other

- [AIRI](https://github.com/moeru-ai/airi) / [XSAI](https://github.com/moeru-ai/xsai) - AI 运行时
- [daily_stock_analysis](https://github.com/ZhuLinsen/daily_stock_analysis) - 每日股票分析
- [agent-browser](https://github.com/vercel-labs/agent-browser) - 浏览器 AI agent
