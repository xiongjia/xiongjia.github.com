...
### sync 详解：让目标对齐 + 增量同步的原理

`rclone sync src:path dst:path` 的官方语义是**把源同步到目标，只修改目标一端**
（changing the destination only）：

- 数据流仍是单向（只从源读、往目标写），但目标与源**完全对齐**：源有目标没
  有的 → 复制；两边都有但判定不同（size / modtime / hash）的 → 覆盖；目标有源没有的 → **删除**
- 因为会删目标端多余对象，是**破坏性命令**（可能丢数据），务必先 `--dry-run`
  或 `--interactive` 预览确认；且出现任何错误时 rclone 不会执行删除
- 被 filter 排除的文件默认**不会**被删（除非加 `--delete-excluded`）
- 不想删目标多余文件就用 `copy`，它是 sync 的非破坏版本

**为什么 sync 能"增量"同步**：它并不是盲目全量重传，而是先**对账、再只动
差异**：

1. **列清单**：先通过两端后端的 List API（S3 系即 ListObjects）拿到源、目标
   各自的对象清单：key + 大小 + 修改时间（+ 校验和）
1. **算差异**：在本地内存里对比两份清单，把每个对象归入四类 —— 仅源有
   （新增）、两边都有但 size / modtime（或 hash）不同（变更）、仅目标有
   （多余）、完全相同（跳过）
1. **只动差异**：仅对"新增 / 变更"发起实际数据传输，对"多余"执行删除，
   完全相同的对象**零数据传输、直接跳过**
1. **幂等可重跑**：第二次运行同一命令时，上次已同步的对象会被识别为"相同"
   而直接跳过，只补新产生的增量 —— 所以**增量同步不需要任何"断点 / 状态记
   录"**，每次都是从两端当前现状对账得出差异，天然支持"切流量后用 sync 补
   齐窗口期增量"

**"相同"怎么判断**（决定哪些对象被跳过，即增量同步的依据）：

| 判断依据                  | 说明                                                                             |
| ------------------------- | -------------------------------------------------------------------------------- |
| 默认：**大小 + 修改时间** | 两端 size 与 modtime 一致（容差 `--modify-window`，默认 1ns）即视为相同          |
| `--checksum`（-c）        | 改为按 **大小 + 校验和** 判断（S3 系小对象 ETag=MD5 可用；multipart 大对象不是） |
| `--size-only`             | 只比大小，快但不严格                                                             |
| `--update`（-u）          | 目标端 mtime 比源新的文件跳过，适合目标有更新、不能回滚的场景                    |
| `--ignore-times`（-I）    | 不跳过任何"看似相同"的对象，无条件全量传输                                       |

相关安全参数：`--check-first` 先完成全部对账再开始传输 / 删除（缩小风险
窗口）、`--max-delete` 限制单次最多删除对象数、`--backup-dir` 把被覆盖 /
删除的对象先移入备份目录，作为 sync 的"后悔药"。

> S3 系场景的细节：modtime 比较依赖 rclone 上传时写入的 `x-amz-meta-mtime`
> 元数据（浮点秒、1ns 精度），该元数据不在 List 响应里，默认每个对象要多发
> 一次 HEAD 请求才能读到（对象多时是笔不小的开销）。若对象由第三方工具上传、
> 没有该元数据，或想省掉 HEAD 开销，可用 `--use-server-modtime` 改用服务端
> Last-Modified（上传时间）比较，或改 `--checksum` 按校验和判定。
