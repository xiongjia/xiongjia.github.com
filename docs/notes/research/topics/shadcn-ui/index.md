...
基于本机实测（Node v24.16.0 / pnpm 11.25.0 / Vite 8.2.2 / Tailwind CSS v4 /
TypeScript 7.0.2 / shadcn CLI 4.20.1，Vite react-ts 模板，macOS arm64）。
...
| 3        | [进阶使用](./advanced.md)             | 组件更新（diff/overwrite）、主题定制（oklch 变量/radius/dark）、表单集成（RHF + zod）、registry 机制与框架差异        |
...
1. **进阶使用** → [进阶使用](./advanced.md)：更新组件、主题定制、表单集成、
   自定义 registry，以及 Vite 脚手架下的实测要点
...
> 版本细项与更新基准见 [setup.md 第 1 节](./setup.md)（两处保持一致）。

## 实测要点（详细见各篇）

- `init`/`add` 前先装 Tailwind v4 并配好 `@/*` 别名（paths 用相对写法
  `"./src/*"`），否则 preflight 报 `No Tailwind CSS configuration found` /
  `Could not find valid path aliases`
- 根 `tsconfig.json`（solution-style）**也要**配同样的 paths —— CLI 靠它把
  组件写进 `src/`；只配 tsconfig.app.json 会落到项目根的字面 `@/components/ui/`
