...
1. **理解入口点**
   - 阅读 `external/jellyfin/Jellyfin.Server/Program.cs` — 了解启动流程
   - 阅读 `external/jellyfin/Jellyfin.Server/CoreAppHost.cs` — 应用主机
   - 阅读 `external/jellyfin/Jellyfin.Server/Startup.cs` — ASP.NET Core 配置

### 阶段 2: 理解核心机制

2. **应用主机 (ApplicationHost)**

   - `external/jellyfin/Emby.Server.Implementations/ApplicationHost.cs` — 核心生命周期管理
   - 了解 DI 容器注册流程
   - 了解插件加载机制

1. **依赖注入**

   - `external/jellyfin/Emby.Server.Implementations/ApplicationHost.cs` 中的 `RegisterServices()` 方法
   - 使用 ASP.NET Core 内置 DI (`Microsoft.Extensions.DependencyInjection`)

1. **插件系统**

   - `external/jellyfin/MediaBrowser.Common.Plugins.IPlugin` — 插件接口
   - `external/jellyfin/Emby.Server.Implementations/Plugins/` — 插件加载和发现
   - `external/jellyfin/MediaBrowser.Common.Plugins.BasePlugin` — 插件基类

### 阶段 3: 理解核心业务

5. **媒体库系统**

   - `external/jellyfin/Emby.Server.Implementations/Library/` — 媒体库扫描和管理
   - `external/jellyfin/MediaBrowser.Controller.Entities/` — 媒体实体 (Movie, Series, Episode 等)

1. **API 层**

   - `external/jellyfin/Jellyfin.Api/Controllers/ItemsController.cs` — 媒体项 API
   - `external/jellyfin/Jellyfin.Api/Controllers/UserController.cs` — 用户管理
   - `external/jellyfin/Jellyfin.Api/Controllers/VideosController.cs` — 视频流

1. **元数据提供者**

   - `external/jellyfin/MediaBrowser.Providers/` — 从 TMDB, IMDB 等获取元数据
   - `external/jellyfin/MediaBrowser.LocalMetadata/` — 从本地文件 (NFO) 解析元数据

### 阶段 4: 深入功能

8. **媒体编码和流媒体**

   - `external/jellyfin/MediaBrowser.MediaEncoding/` — FFmpeg 集成
   - `external/jellyfin/Jellyfin.Api/Controllers/DynamicHlsController.cs` — HLS 流

1. **会话和播放状态**

   - `external/jellyfin/Emby.Server.Implementations/Session/` — 会话管理
   - `external/jellyfin/Jellyfin.Api/Controllers/PlaystateController.cs` — 播放状态上报
