    ...
    for spec in specs:
        for part in re.split(r"\s+\+\s+", spec):
            ...
            if name not in TASKS:
                raise BotError(f"unknown task {name!r}; available: {', '.join(sorted(TASKS))}")
            parsed.append((name, rest))
    return parsed


def aggregate_commit(tasks_run: list[tuple[str, dict]]) -> tuple[str, list[str]]:
...
def do_submit(
    ...
    subject, body = aggregate_commit(tasks_run)
