...
| `BOT_API_HOST` / `BOT_API_PORT`                                       | Bot Remote API bind (default 0.0.0.0:8100; pin 127.0.0.1 for local-only — no auth)                                                | `0.0.0.0` / `8100`                        |
| `BOT_API_LOG_DIR`                                                     | Runtime data dir for the API: history JSONL + uploads staging + `cron-state.json` (absolute or repo-relative)                     | `.bot-api`                                |
| `BOT_API_STARTUP_CLEANUP`                                             | `false` skips `poe bot cleanup` of stale worktrees at API startup (tests)                                                         | `true`                                    |
| `BOT_API_CRON_ENABLED`                                                | `false` disables cron scheduling (mkdocs.yml `extra.bot.cron`) entirely — tests / maintenance                                     | `true`                                    |
| `TG_BOT_TOKEN` / `TG_ALLOWED_USER_IDS` / `TG_MODE` / `TG_WEBHOOK_URL` | Telegram bot config (allowlisted users; empty token disables the bot)                                                             | empty / empty / `polling` / empty         |
