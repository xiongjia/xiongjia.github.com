...
随手收集 ──> inbox（暂存）──> AI 批量整理 ──> 统一归档 ──> Anki 导出复习（可选）
   (几秒)       (单一文件)     (攒够一批后)   (archive/周文件) (仅当用户要时)
```
...
└── scraps/                   # 本计划的收集与归档区
    ...
    └── archive/              # 📚 归档区（按 ISO 周分文件，有归档的周才建）
        ├── index.md          # 归档索引：周列表 + 字段说明
        └── 2026-w33.md       # 周文件示例（条目按 date 落进对应周）
```

- **按 ISO 周归档**：卡片按条目 `date` 所在 ISO 周写入 `archive/<YYYY-www>.md`
  （如 `2026-w33.md`），只在有归档的周生成文件、不强制每周都有；用 `type` +
  `tags` 区分知识点，周文件天然小、单文件不再无限膨胀
...
| ③ 查看         | 打开站点归档索引（scraps/archive/）                | 任何时间                      | archive/index.md 周列表 + 字段说明                                   |
...
1. 写入归档：按条目 `date` 所在 ISO 周确定周文件 `scraps/archive/<YYYY-www>.md`
   （文件不存在则新建，frontmatter 同归档索引约定）；条目追加到文件末尾，不维护
   文件内索引，靠站内全文搜索 + backlinks 插件查找；一次归档跨多周时各条目各落各周
1. **归档后删除**：从 inbox 删除已处理条目（不保留、不建处理日志）
1. 更新 `scraps/index.md` 的「上次整理」日期，并把新周行追加到
   `archive/index.md` 的「归档列表」（一行/周，已有周不重复、不计数；手动整理
   后也应更新）
...
- **搜索范围**：在所有 `archive/*.md` 周文件内搜索 `### <关键词>`（跨周查重，
  同一表达不会因跨周重复建卡）
...
> 周文件 frontmatter 的 `title` 带周号（如 `2026-w33`）；「字段说明」等公共说明
> 只维护在 `archive/index.md`，周文件内不重复。
...
- `英义`: 英英释义，单词类（word / phrasal-verb / collocation / idiom）**必填**；
  grammar / sentence 不强制
- `记忆`: 词根/词源/助记法，**选填**——对记忆有帮助才写，特别简单的词不用
- 其余字段按 `type` 选填：`word` 有发音/含义/语境/原句/造句/同义反义；
  `phrasal-verb` / `collocation` / `idiom` 有含义/例句/替换/易混/对比；`grammar`
  有规则/例句/易错点；`sentence` 有结构拆解/翻译/难点/仿写

## 索引与查找

- **不做实时条目索引**：周文件顶部不维护「条目索引」列表（避免每次整理的编辑
  热点和 git 冲突）；周列表只在 `archive/index.md` 维护一行/周
- 查找靠：站内全文搜索（MkDocs `search` 插件）+ backlinks 插件互链
- `scraps/index.md` 记录「上次整理：YYYY-MM-DD」，作为触发条件（≥ 2 周）的
  可观测依据；**手动编辑 archive 后也应更新该字段**
- **归档可用性**：搜索靠全文搜索，试点时实测 `###` 条目可命中；backlinks
  基于页面间链接，周文件内锚点可能不产生反向链接，**不依赖它**；周文件
  frontmatter 加 `hide: navigation`；单周文件量小，渲染无压力
...
- archive/ 周文件用户可随时手改；AI 下次整理前先读相关周文件，**尊重已有内容**
  ——不覆盖、不「纠正」用户改过的地方
...
- **主动触发**：`/skill:enu-organize quiz [范围]` → AI 出 5–10 题（英译中/中译英/填空）
  并批改；`/skill:enu-organize review <tag>` → AI 先扫所有 archive/ 周文件的 tags
  给出可选项，再筛出列出

  `quiz` 范围参数（可选，缺省 = 最近一次整理涉及的所有周文件）：

  - `quiz` → 最近一批（默认，刚学的先测；一批可能跨多个周文件）
...
- **卡片模板**：每 `type` 一个 Anki note type（避免跨类型撞名），每个 note 配
  **2 个卡模板**（同一知识点双向复习，Anki 一个 note 自动生成多张卡）：
  - 识别卡：正面 = 词/词组（+ 音标），背面 = 含义/英义/记忆/语境/原句/造句
  - 产出卡（中译英）：正面 = 中文含义，背面 = 词/词组 + 音标
    正反面字段映射（按 type）：

| type           | Anki 正面                | Anki 背面                               |
| -------------- | ------------------------ | --------------------------------------- |
| `word`         | `cumbersome` /ˈkʌmbəsəm/ | 含义 + 英义 + 记忆 + 语境 + 原句 + 造句 |
...
- **导入方式（手工 + 说明文档）**：不做 AnkiConnect 自动上传，生成文件 + 说明，
  用户手动导入：
  - 默认 `genanki` 生成 `.apkg`（输出到 gitignore 目录 `.anki/`），双击导入 Anki；
    apkg 内按 note guid 去重，重复导入安全
  - 兜底 CSV：UTF-8 with BOM，每 type 一个文件，第一字段 = 该 type 去重 key，
    导入勾选 "Update existing notes when first field matches"
  - 说明文档写入使用指南（`scraps/index.md`）Anki 章节：生成命令、导入步骤、
    去重行为、状态流转、导入后同步 AnkiWeb/手机
  - 不做 AnkiConnect：需本机 Anki 运行 + 插件；AnkiWeb 无公开上传 API
- **quiz 题目卡**：quiz（交互出题）保持动态；卡片化只做「识别 + 中译英」双向卡，
  **不固化** quiz 的填空/选词干扰项（干扰项应动态生成，固化价值低、会过期）
...
- **生成策略（全量、无状态）**：每次从 `scraps/archive/*.md` 全量解析生成导出文件，
  **不做增量 cache**：
  - 输出到 gitignore 目录（如 `.anki/`，加 .gitignore），生成物不提交；卡片量小
    （每周几张、一年约 26 个周文件），全量解析成本可忽略
  - 幂等靠三层去重（归档 key + `status: new` 筛选 + CSV 导入 update-existing），
    重复导出只会更新、不会重复
  - cache（不提交目录记录上次导出游标）**当前不需要**：与 `status` 字段
    （随卡片提交的状态标记）职责重复；cache 丢失要重建、用户手改卡后 hash
    失效，复杂度换来的收益在千张以下趋近于零
  - 何时再引入 cache：卡片达数千张、或放弃 update-existing 需要精确增量时再评估
...
- 链接：相对链接（如 `./scraps/archive/`、`./scraps/archive/2026-w33.md`）
...
- [x] 建立 `scraps/` 结构（index.md / inbox.md / archive/ 周文件）
...
- [x] 验证 archive/ 周文件渲染效果（生产构建已生成 scraps 页面）
...
- **按周归档（定稿）**：卡片按条目 `date` 落进 ISO 周文件 `archive/<YYYY-www>.md`，
  只在有归档的周生成；用 `type` + `tags` 区分，单文件不再无限膨胀；周列表在
  `archive/index.md` 维护一行/周（skill arch 时同步更新）
- **超长处理（已废弃）**：旧设计「单文件超 5000 行改名归档」已废弃 —— 按周分文件后
  每个周文件天然小，无需拆分
- **回顾与周复盘完全解耦**：用户也做不到 weekly → 不做周复盘；回顾方式灵活
  （如攒一批后集中看一遍 archive，或按 tag 筛选）；复习可交给 Anki（可选）
- **归档即清理**：条目归档后直接从 inbox 删除，不保留已处理痕迹 —— archive/
  周文件就是唯一沉淀，维护成本最低
