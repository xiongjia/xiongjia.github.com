...
- subject = main change + abbreviated extras (length-controlled):
  `[bot] feat(weight): record 81.5 kg + add text-moment + refresh summary`
- body lists each task's conventional message:
  ```
  [bot] feat(weight): record 81.5 kg on 2026-08-11
  [bot] feat(moment): add text "晨跑5km"
  [bot] docs(health): refresh summary
  ```
- PR title = subject; body = task summary + changed files.
...
- Conventional commits, subject prefixed with `[bot]` (task template):
  - `[bot] feat(weight): record 82 kg on 2026-08-11`
  - `[bot] feat(moment): add text "晚饭打卡"`
  - composed: `[bot] feat(weight): record 82 kg + add text-moment`, body lists each
    task's message
- **PR title also carries `[bot]`**: auto-merge squashes with the PR title as
  ...
  in master history.
- PR body = task summary + changed files.
...
| `.pi/`, `.claude/`, `.pi-subagents/`                         | no build impact                                       | ignore                                                                                                                                                                                                                                                                                        |
