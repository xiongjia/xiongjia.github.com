...
- **English in code**: Avoid Chinese in source code of any language — comments, docstrings, error messages and console output stay in English unless there is a real need, e.g. Chinese display strings for UI/rendering (page content rendered for Chinese readers, prompts passed to a Chinese-speaking AI) or data literals that must match Chinese data files (e.g. the reading-items field keys). Chinese is fine in Markdown docs/content; keep code files English.
